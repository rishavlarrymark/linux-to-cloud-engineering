...
- **Fix:** `df -h` → identify full mount → investigate large paths  
...
- **Fix:** `du -sh /*` → drill down into largest path  
...
- **Fix:** `lsof | grep deleted` → restart offending process  
...
- **Fix:** `lsblk` → verify device → mount correctly  
...
- **Fix:** `mount` → verify mount location → remount if required  
...
- **Fix:** `ncdu -x /` → identify large directories → clean safely  
...
- **Fix:** `df -h` → `du -sh` → check mount → verify permissions  
